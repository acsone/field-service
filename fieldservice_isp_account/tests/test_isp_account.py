...
class FSMISPAccountCase(SavepointCase):
    ...
    def _process_order_to_invoices(self, order):
        ...
        bill = self.AccountInvoice.search(
            [("type", "=", "in_invoice"), ("fsm_order_id", "=", order.id)]
        )
        ...
        invoice = self.AccountInvoice.search(
            [("type", "=", "out_invoice"), ("fsm_order_id", "=", order.id)]
        )
